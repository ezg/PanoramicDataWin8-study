...
namespace DynamicExpresso.Parsing
{
	internal class Parser
	{
		...
		Parser(ParserArguments arguments)
		{
			...
			//_bindingCase = arguments.Settings.CaseInsensitive ? BindingFlags.IgnoreCase : BindingFlags.;
			//_memberFilterCase = arguments.Settings.CaseInsensitive ? Type. : Type.FilterName;
		...
		MemberInfo FindPropertyOrField(Type type, string memberName, bool staticAccess)
		{
			...
			foreach (Type t in SelfAndBaseTypes(type))
			{
                ...
                MemberInfo[] members = t.GetMembers(flags).Where(mi =>mi.Name == memberName).ToArray();
				if (members.Length != 0)
					return members[0];
			}
			return null;
		...
		MethodData[] FindMethods(Type type, string methodName, bool staticAccess, Expression[] args)
		{
			...
			foreach (Type t in SelfAndBaseTypes(type))
			{
                ...
                MemberInfo[] members = t.GetMethods(flags).Where(mi => mi.Name == methodName).ToArray();
